...
class ArticleTest < ActiveSupport::TestCase
  ...
  def test_creating_permalink
    a = @blog.articles.new(FactoryGirl.build(:permalink_less_article)).save
  ...
  def test_creating_permalink_unusual_characters
    a = @blog.articles.new(FactoryGirl.build(:article_with_unusual_chars)).save
    assert_equal "this-is-a-title-456", a.permalink
  end

  def test_duplicate_permalink
    a = @blog.articles.new(FactoryGirl.build(:permalink_less_article)).save
    a2 = @blog.articles.new(FactoryGirl.build(:permalink_less_article)).save
    assert_equal "this-is-a-title-123-1", a2.permalink
  end

  def test_no_title
    a = @blog.articles.new(FactoryGirl.build(:permalink_less_article))
    a.title = nil
    assert !a.save
  ...
  def test_publishing
    a = @blog.articles.new(FactoryGirl.build(:unpublished_article))
  ...
  def test_republishing
    a = @blog.articles.new(FactoryGirl.build(:article))
    a.set_published(true, "ric")

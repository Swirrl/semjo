...
class PageTest < ActiveSupport::TestCase
  ...
  def test_creating_permalink
    a = @blog.pages.new(Factory.build(:permalink_less_page)).save
  ...
  def test_creating_permalink_unusual_characters
    a = @blog.pages.new(Factory.build(:page_with_unusual_chars)).save    
    assert_equal "this-is-a-title-456", a.permalink
  end
  
  def test_duplicate_permalink
    a = @blog.pages.new(Factory.build(:permalink_less_page)).save               
    a2 = @blog.pages.new(Factory.build(:permalink_less_page)).save
    assert_equal "this-is-a-title-123-1", a2.permalink
  end
  
  def test_no_title
    a = @blog.pages.new(Factory.build(:permalink_less_page))
    a.title = nil       
    assert !a.save    
  ...
  def test_publishing
    a = @blog.pages.new(Factory.build(:unpublished_page))
  ...
  def test_republishing
    a = @blog.pages.new(Factory.build(:page))
    a.set_published(true, "ric")
